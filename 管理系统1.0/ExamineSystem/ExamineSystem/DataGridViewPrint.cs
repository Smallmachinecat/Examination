...
namespace ExamineSystem
{
    public class DataGridViewPrint
    {
        ...
        public void Printer(DataGridView dataview, PrintDocument printDoc, string[] footer)
        {
            this.footer = footer;
            ...
            width = PageWidth - LeftMargin - RightMargin+100;
        ...
        public bool Print(Graphics g, string cb1,string cb2)
        {
            InitPrint();
            DrawTable(g);
            DrawHeader(g);
            ...
            try
            {
                ...
                g.DrawString(cb1+" "+cb2, new Font("宋体", 30, FontStyle.Regular), Brushes.Black, 230, 10);

                g.DrawString("签名：", new Font("宋体", 10, FontStyle.Regular), Brushes.Black, 100, 1120);
                g.DrawString("盖章：", new Font("宋体", 10, FontStyle.Regular), Brushes.Black, 630, 1120);
                g.DrawString(pagestr, font, Brushes.Black, new
               PointF((PageWidth / 2) - g.MeasureString(pagestr, font).Width+10, PageHeight - (BottomMargin / 2) - g.MeasureString(pagestr, font).Height));
                g.RotateTransform(60f);
                Brush b = new SolidBrush(Color.FromArgb(60, Color.Black));
                g.DrawString("江苏机电大类学业水平测试", new Font("宋体", 70, FontStyle.Regular), b,100, -100);
